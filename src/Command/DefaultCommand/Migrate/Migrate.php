...
{
    const MIGRATE_COMMAND_ALL = 'all';

    protected $defaultModelPath;

    protected $defaultModelNamespace;

    protected $defaultModel;

    protected $defaultMysqlConfig;
    ...
    {
        $this->defaultModel = Config::getInstance()->getConf('MIGRATE.BASE_MODEL');
        $this->defaultModelNamespace = Config::getInstance()->getConf('MIGRATE.MODEL_NAMESPACE');
        $this->defaultModelPath = Config::getInstance()->getConf('MIGRATE.MODEL_PATH');
        $mysqlConfigTemp = Config::getInstance()->getConf('MIGRATE.MYSQL_CONFIG');
        if (is_array($mysqlConfigTemp)) {
            $this->defaultMysqlConfig = $mysqlConfigTemp;
        } else {
            $this->defaultMysqlConfig = Config::getInstance()->getConf($mysqlConfigTemp);
        }
    }
    ...
    public function execSql($tableName, $sql)
    {
        ...
        $mysql = new \mysqli(
            $this->defaultMysqlConfig['host'],
            $this->defaultMysqlConfig['user'],
            $this->defaultMysqlConfig['password'],
            $this->defaultMysqlConfig['database'],
            $this->defaultMysqlConfig['port']
        );
     ...
     * @return string
     */
    protected function analysisProperty($model): string
    {
    ...
    protected function tableNameMap()
    {
        ...
        foreach ($this->readClassName() as $className) {
            if (class_exists($className)) {
                ...
                if ($this->getTopParentClass($reflection) == $this->defaultModel)
                {
    ...
    protected function getTopParentClass(\ReflectionClass $reflectionObj)
    {
        ...
        if ($parentClass->name != $this->defaultModel) {
    ...
    protected function readClassName($basePath = null): array
    {
        ...
        $returnArray = [];
        if (!$basePath)
            $basePath = $this->defaultModelPath;
        ...
        foreach ($direction as $item) {
            if ($item == '.' || $item == '..')
                ...
                $modelClass = str_replace(
                    [$this->defaultModelPath, '/\\', '/', '.php'],
                    [$modelNamespace, '\\', '\\', ''],
                    $basePath . '/' . $item
                );
                if ($modelClass != $this->defaultModel)
    ...
    public function help(array  $args):? string
    {
        ...
        $allValue = self::MIGRATE_COMMAND_ALL;
